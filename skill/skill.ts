...
async function run(opts: { force?: boolean } = {}): Promise<void> {
  ...
  const [hnResult, redditResult, githubResult] = await Promise.allSettled(fetchTasks)
  ...
  if (enabledSources.hn && hnResult.status === 'rejected') {
    console.warn('[newspaper-fetch] HN fetch failed:', hnResult.reason)
  }
  if (enabledSources.reddit && redditResult.status === 'rejected') {
    console.warn('[newspaper-fetch] Reddit fetch failed:', redditResult.reason)
  }
  if (enabledSources.github && githubResult.status === 'rejected') {
